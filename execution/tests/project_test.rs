...
fn hustle_where(relation1: Relation) -> Relation {
    let project_operator = Project::pure_project(
        relation1.clone(),
        vec![Column::new(
            "a".to_string(),
    ...
    let print_node_before = Rc::new(Node::new(
        print_operator.clone(),
        vec![project_node_before.clone()],
    ));
    let project_node_after = Rc::new(Node::new(
        project_operator_after.clone(),
        vec![print_node_before.clone()],
    ));
    let print_node_after = Rc::new(Node::new(
        print_operator_after.clone(),
        vec![project_node_after.clone()],
    ));
